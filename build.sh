#!/usr/bin/env bash
# StandartOS build helper script
...
# Releaser path
RELEASE_HELPER_PATH="vendor/standart/private/release.sh"
...
while true; do
    case "$1" in
        ...
        -r|--release|r|release) FLAG_STANDARTOS_RELEASE=y;;
...
if [ ! -d "$DIR_ROOT/vendor/standart" ]; then
...
PIXELBUILDS_VERSION="$(cat $DIR_ROOT/vendor/standart/config/version.mk | grep 'STANDARTOS_VERSION := *' | sed 's/.*= //') "
...
if [ "$FLAG_STANDARTOS_RELEASE" ]; then
